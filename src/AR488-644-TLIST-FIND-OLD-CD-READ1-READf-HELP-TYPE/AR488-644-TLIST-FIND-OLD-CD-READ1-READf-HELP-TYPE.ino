...
// SD chip select pin
const uint8_t chipSelect = 4;     // SS pin on Pandauino 644-Narrow
...
void tek_READ_file() {              // Read entire currently open file based on file type
  ...
  if ((f_type == 'P') || (f_type == 'D')) {  
    ...
    cout << "LAST file - no data\n";

  } else if (f_type == 'N') {
    cout << "No file is open\n";

  } else { // should be BINARY "H"
    char buffr[bin_buffer_size];
    char binary[33]; // output buffer size = bin buffer size / 2 plus NULL
...
char tek_FIND(int num) {
  ...
  /*  FINDs and OPENs file (num)
       Returns: string result= "A" for ASCII, "H" for HEX, "N" for Not Found
  ...
  for (int index = 0; index < nMax; index++) { //SdFat file index number
    ...
    if (!file.isSubDir() && !file.isHidden()) {
      ...
      if (filenumber == num) {
        ...
        // all BINARY and SECRET files are in HEX format, else ASCII Program or ASCII Data
        if (f_name[7] == 'B' || f_name[25] == 'S') {
...
static cmdRec cmdHidx [] = {
  ...
  { "readf",       3, readf_h     },
  { "read1",       3, read1_h     },
  { "help",        3, help_h      },  //help for Tek 4924 commands
  { "old",         3, old_h       }
...
// ************ Tek 4924 command_h in Prologix ++ format *********************
// ***** CD changes the directory for use by the other commands *****
...
// ***** READf sends entire ASCII or BINARY files to 4050 computer based on file type *****
...
// ***** READ1 reads one data element based on data type *****
...
// ***** OLD sends ASCII program text to 4050 computer *****
...
void find_h(char *params) {
  // find FILE x in current directory
  // extract filenumber parameter
  char *param;
  uint16_t val;
  if (params != NULL) {
...
void tlist_h(char *params) {
  // TLIST in numeric order all files in current directory
...
// ********** END of Tek 4924 command_h routines **********************
