...
#define FILENAME_FNUM_SIZE 7
#define FILENAME_FTYPE_SIZE 8
#define FILENAME_FUSAGE_SIZE 5
#define FILENAME_FSIZE_SIZE 7
#define FILENAME_FDESC_SIZE file_header_size - 30
...
class SDstorage {
  ...
  private:
    ...
    char directory[DIR_NAME_SIZE] = "/root/"; //allow up to ten character directory names plus two '/' and NULL terminator
...
//    #include <SD.h>
    SdFat sd;
//    SdFs sd;
    typedef SdFile tekSdFile;
  #else
    SdFat sd;
    typedef File tekSdFile;
  #endif
...
//    void stgc_0x7D_h();
    ...
    bool searchForFile(uint8_t filenum, tekSdFile& fileObj);
    uint8_t getLastFile(tekSdFile& fileObj);
    uint8_t binaryRead();
    uint8_t binaryCopy();
    bool binaryWrite();
    bool renameFile(tekSdFile& fileObj, char ftype, char fusage);
    bool makeNewFile(tekSdFile& fileObj, uint16_t filelength);
    void setFileInfo(tekSdFile filehandle);
