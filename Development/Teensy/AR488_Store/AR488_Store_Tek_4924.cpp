...
void TekFileInfo::getFtypeStr(char * typestr){
  ...
  while (tekFileTypes[i].idx) {
    if (tekFileTypes[i].idx == ftype) {
      strncpy(typestr, tekFileTypes[i].desc, FILENAME_FTYPE_SIZE);
      break;
    }
    i++;
  }
...
//        strncpy(usagestr, tekFileUsages[i].desc, strlen(tekFileUsages[i].desc));
        strncpy(usagestr, tekFileUsages[i].desc, FILENAME_FUSAGE_SIZE);
        break;
      }
      i++;
    }
...
//  strncpy(sfname, filename, strlen(filename));
  strncpy(sfname, filename, file_header_size-1);
  param = strtok(sfname, " ");
...
bool SDstorage::findFile(uint8_t fnum){
  ...
  if (searchForFile(fnum, findfile)) { // Returns filehandle if found
    ...
    if (f_type) {
      strncpy(path, directory, DIR_NAME_SIZE);
      strncat(path, f_name, file_header_size);
 ...
 */
bool SDstorage::searchForFile(uint8_t filenum, tekSdFile& fileObj){
...
uint8_t SDstorage::getLastFile(tekSdFile& fileObj) {
  tekSdFile dirObj;
  char fname[file_header_size];
...
/***** Rename a file *****/
bool SDstorage::renameFile(tekSdFile& fileObj, char ftype, char fusage) {
...
/***** Make a file NEW *****/
bool SDstorage::makeNewFile(tekSdFile& fileObj, uint16_t filelength) {
  ...
  if (ftype != 'L') {   // Cannot kill the LAST file!
    ...
    strncpy(path, directory, DIR_NAME_SIZE);
    strncat(path, fname, file_header_size);
    fileObj.close();
